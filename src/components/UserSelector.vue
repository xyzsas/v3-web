...
let showResult = $ref(false), allSelected = $ref(false)
...
<template>
  <div class="fixed top-0 w-11/12 sm:w-96 bg-white min-h-screen all-transition py-10 px-4" :class="model.show ? 'right-0' : '-right-96'">
    ...
    <div v-if="mode === 2" class="flex flex-col my-2 font-mono">
      <div class="flex items-center m-1">
        年级
        <input class="border rounded py-1 px-2 mx-1" v-model="filter['年级']">
      </div>
      <div class="flex items-center m-1">
        班级
        <input class="border rounded py-1 px-2 mx-1" v-model="filter['班级']">
      </div>
      <div class="flex items-center m-1">
        姓名
        <input class="border rounded py-1 px-2 mx-1" v-model="filter['姓名']">
      </div>
      <div class="flex items-center m-1">
        学号
        <input class="border rounded py-1 px-2 mx-1" v-model="filter['学号']">
      </div>
    </div>
