...
function change (cur) { // select
  if (value.space && affair.variable['$' + cur] <= 0 && !selected[cur]) return
  if (selected[cur]) {
    delete selected[cur]
    if (value.space) affair.variable['$' + cur]++
  } else {
    selected[cur] = 1
    if (value.space) affair.variable['$' + cur]--
  }
...
<template>
  <div class="px-3 py-2">
    ...
    <p class="mt-3 mb-1 text-sm all-transition" :class="affair.ok[key] ? 'text-gray-300' : 'text-red-500'">
      ...
      <template v-else>
        <span class="mr-1" v-if="value.min">最少选择{{ value.min }}项</span><span class="mx-1" v-if="value.max">最多选择{{ value.max }}项</span>
      </template>
    </p>
    <ul class="w-full bg-white lg:w-1/2 border border-gray-200">
      <li v-for="(o, i) in value.options" @click="change(o['#'])" class="flex py-2 items-center justify-between relative cursor-pointer" :class="value.space && affair.variable['$' + o['#']] <= 0 ? 'text-gray-600' : 'font-bold'">
        ...
        <div class="mx-3 z-10"><span v-if="value.space">({{ affair.variable['$' + o['#']] }})</span></div>
      </li>
    </ul>
