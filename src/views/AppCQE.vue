  ...
  <div class="flex flex-col items-start p-4 sm:p-10">
    ...
    <template v-if="current === 1">
      <div v-for="(v, k) in D.思想品德" class="w-full">
        <CQECard v-if="k != 'done'" :term="term" :key="term" v-model="D.思想品德[k]" page="思想品德" :title="k" :content="T.思想品德[k]" :basis="T.思想品德.评价主要依据" @done="d => done.思想品德[k] = d"/>
      </div>
    </template>
    <template v-if="current === 2">
      <div v-for="(v, k) in D.学业水平" class="w-full">
        <CQECard v-if="k != 'done'" :term="term" :key="term" v-model="D.学业水平[k]" page="学业水平" :title="k" :content="T.学业水平[k]" :basis="T.学业水平.评价主要依据" @done="d => done.学业水平[k] = d"/>
      </div>
    </template>
    <template v-if="current === 3">
      <div v-for="(v, k) in D.身心健康" class="w-full">
        <CQECard v-if="k != 'done'" :term="term" :key="term" v-model="D.身心健康[k]" page="身心健康" :title="k" :content="T.身心健康[k]" :basis="T.身心健康.评价主要依据" @done="d => done.身心健康[k] = d"/>
      </div>
    </template>
    <template v-if="current === 4">
      <div v-for="(v, k) in D.艺术素养" class="w-full">
        <CQECard v-if="k != 'done'" :term="term" :key="term" v-model="D.艺术素养[k]" page="艺术素养" :title="k" :content="T.艺术素养[k]" :basis="T.艺术素养.评价主要依据" @done="d => done.艺术素养[k] = d"/>
      </div>
    </template>
    <template v-if="current === 5">
      <div v-for="(v, k) in D.社会实践和劳动教育" class="w-full">
        <CQECard v-if="k != 'done'" :term="term" :key="term" v-model="D.社会实践和劳动教育[k]" page="社会实践和劳动教育" :title="k" :content="T.社会实践和劳动教育[k]" :basis="T.社会实践和劳动教育[k][2]" @done="d => done[社会实践和劳动教育] = d"/>
      </div>
    </template>
