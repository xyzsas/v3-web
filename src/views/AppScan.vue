...
async function result (text) {
  got = text
  ready = await srpc.Y.pub(state.user.token, { scanRes: text, scanResTime: Date.now() }) // submit data
}
...
<template>
  <div class="w-full h-screen flex flex-col items-center justify-center text-white bg-gray-900">
    <h1 class="text-3xl font-bold mb-4">扫 码</h1>
    <p class="mb-10 text-sm">正在填写：{{ label }}</p>
