...
async function submit () {
  loading = true
  const res = await request.post('/xyz/affair/' + route.params.id, affair.data, opt)
...
<template>
  ...
  <div class="min-h-screen p-4 lg:px-20 lg:py-8">
    ...
    <button @click="submit" class="text-white font-bold w-full sm:w-32 py-2 my-8 sm:mx-4 rounded shadow flex items-center justify-center all-transition hover:shadow-md bg-blue-500"><check-icon class="w-6 mr-2" />提交<div class="w-4" /></button>
  </div>
</template>
